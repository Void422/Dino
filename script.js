...
function loadImages(s){
  baseImg=new Image(); bodyMaskImg=new Image(); bellyMaskImg=new Image()
  baseImg.crossOrigin=bodyMaskImg.crossOrigin=bellyMaskImg.crossOrigin="anonymous"
  baseImg.src=s.base; bodyMaskImg.src=s.maskBody; bellyMaskImg.src=s.maskBelly
  let n=0; const ok=()=>{if(++n<3)return; init()}
  baseImg.onload=ok; bodyMaskImg.onload=ok; bellyMaskImg.onload=ok
}
...
function init(){
  ...
  offMain=document.createElement("canvas"); offBody=document.createElement("canvas"); offBelly=document.createElement("canvas")
  offMain.width=w; offMain.height=h; offBody.width=w; offBody.height=h; offBelly.width=w; offBelly.height=h
  ...
  baseBox=findBox(baseImg); bodyBox=findBox(bodyMaskImg); bellyBox=findBox(bellyMaskImg)
  buildBodySet("N"); buildBellySet("N")
...
function pickAlpha(ctx,w,h){
  const d=ctx.getImageData(0,0,w,h).data, a=new Uint8ClampedArray(w*h)
...
function colorRegion(imgData,maskData,hex,original){
  ...
  for(let i=0;i<bd.length;i+=4){
    const ma=md[i+3]; if(ma<=2) continue
    if(bd[i+3]<=10) continue
    const l=light(od[i],od[i+1],od[i+2]); if(l<=keep) continue
    const w=ma/255
...
function buildBodySet(key){
  const list=info.bodySets[key]
  ...
  list.forEach(item=>{
    ...
    const dot=document.createElement("div"); dot.className="dot"; dot.style.background=item.hex||"#fff"
...
function buildBellySet(key){
  const list=info.bellySets[key]
...
/* faster redraw on rotate */
window.addEventListener("orientationchange",()=>setTimeout(drawAll,200))
