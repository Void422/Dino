...
fetch("info.json").then(r=>r.json()).then(j=>{info=j; bodyHex=info.defaults.body; bellyHex=info.defaults.belly; guard=info.defaults.outlineGuard??0.4; return loadAll(info.images)}).then(init)
...
function init(){
  ...
  main=mkCanvas(w,h); layerBody=mkCanvas(w,h); layerBelly=mkCanvas(w,h)
...
function refine(c){
  ...
  const er=erode(alpha,w,h)
  const bl=blur(er,w,h)
...
function buildBody(key){
  ...
  for(const it of list){
    ...
    card.className="swatch"+(it.on?"":" s-off")
    const dot=document.createElement("div"); dot.className="dot"; dot.style.background=it.hex||"#fff"
    const meta=document.createElement("div"); meta.className="meta"
    const title=it.on?`${it.id} • ${it.name}`:`${it.id} • Missing`
    const code=it.on?it.hex:"—"
...
function buildBelly(key){
  ...
  for(const it of list){
    ...
    const dot=document.createElement("div"); dot.className="dot"; dot.style.background=it.hex||"#fff"
    const meta=document.createElement("div"); meta.className="meta"
    const title=it.on?`${it.id} • ${it.name||"Belly"}`:`${it.id} • Missing`
    const code=it.on?it.hex:"—"
    meta.innerHTML=`<div class="name">${title}</div><div>${code}</div>`
    card.appendChild(dot); card.appendChild(meta)
